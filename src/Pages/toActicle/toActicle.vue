    ...
    <div class="container">
      ...
      <div  class="row justify-content-md-center mt-3">
        <div class="col-lg-10">
         ...
         <div class="row">
           <div class="check">
               <p class="text-muted" @click="checkmore" v-if="list.length>3">查看更多评论 <i class="el-icon-caret-bottom"></i></p>
               <p class="text-muted"  v-if="list.length===0" >暂无评论 <i class="el-icon-more"></i></p>
            </div>
         </div>
        </div>
      </div>
...
<script>
import { findId, readConut,commitTo } from "../../api/con";
...
export default {
  ...
  methods: {
    async getId() {
      ...
      //  console.log(result.data.message[0].readcount);
    },
    //获取所有表情
    faceContent() {
   ...
   async commit() {
      if(this.name === null){
        ...
        // console.log(this.obj.img,this.obj.pulisher,this.id,this.obj.pulishcontent)
        //  this.$store.dispatch("committo", this.obj.img,this.obj.pulisher,this.id,this.obj.pulishcontent);
        const results = await commitTo(this.obj.img,this.obj.pulisher,this.id,this.obj.pulishcontent);
        // console.log(results);
        this.obj={pulishcontent:""}
        this.$router.go(0);
       
       
        // this.contentList = contentList.push(obj);
